...
CCflk.sf =st_as_sf(CCflk, coords = c("Longitude","Latitude"), crs = st_crs(4326))     
...
#this one doesn't work for plotting. WHY? #Maybe because it is point data? 
...
mirMonitoring_sp <- st_read('/Users/heidi.k.hirsh/Desktop/MIR Shapefiles/Monitoring Subplots/MonitoringSubplotCoordinates.shp') #, crs=st_crs(26717)) #utm zone 17R
st_crs(mirMonitoring_sp)
test=st_transform(mirMonitoring_sp, crs=st_crs())
...
#compare all layers
# mapview(mirOverall_sp,col.regions='white')+  #most basic
mapview(mirOverall_sp,zcol="MIRSite",alpha.regions=.1)+  #most basic
  mapview(mirContRest_sp,zcol="Type")+
  mapview(mirSegments_sp,col.regions='white',alpha.regions=0)+ #,zcol="Site")+
  mapview(MIR_sf,zcol="MPA_NAME")


names(mirOverall_sp)
names(mirContRest_sp)
names(mirSegments_sp)
names(mirMonitoring_sp)


#plot MIR sites with CC data
mapview(mirOverall_sp)+mapview(mirSegments_sp,zcol="Site")+
  mapview(CCflk.sf,zcol='pH_measured')
