#model flk!
#this is just the edited version to try to further understand region (or distance index)
...
library(GGally)
library(geosphere) #distance between points (get distm function)
library(leaflet)
...
#save CCfull with region (distIndex)
# write.csv(CCfull, file='/Users/heidi.k.hirsh/Desktop/CCfull+distance_Oct4.csv')

CCfull = CCfull[which(CCfull$Zone=='Inshore'),] #limit to only inshore
