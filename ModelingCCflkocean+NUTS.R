#model flk! with NUTRIENTS TOO!

#limit plots to DIC and TA and add OCE as variable (oceanic chemistry?)
##10/9/2023
...
# CCfull = read.csv('/Users/heidihirsh/Desktop/CCmodel_17OCT2022off.csv')
# CCfull = read.csv('/Users/heidihirsh/Desktop/CCmodel_27OCT2022off.csv')
# CCfull = read.csv('/Users/heidihirsh/Desktop/FLK_data/CCmodel_halloween.csv')
# CCfull = read.csv('/Users/heidi.k.hirsh/Desktop/CCmodel_allYears_nov9.csv')
# dim(CCfull) #9254   88 (WHY NOT MORE?)
# 3150    91 for 2016 only 

CCfull.all = read.csv('/Users/heidi.k.hirsh/Desktop/CCmodel_allYears+Nuts_13nov2023.csv') #new file with nutrients added
dim(CCfull.all) #9639  100



#how many rows have NA nutrients (should lack all nutrient columns -   "SST"   "SSS"  "Chla"     "Phphytn"     "NH4"       "PO4"    "NO3_NO2"     "NO2"     "NO3"    "Si")
# CCfull = CCfull.all[which(!is.na(CCfull.all$Chla) & !is.na(CCfull.all$Phphytn) & !is.na(CCfull.all$NH4) & !is.na(CCfull.all$PO4) & !is.na(CCfull.all$NH4)),]
Nutcols = c("SST","SSS","Chla","Phphytn","NH4","PO4","NO3_NO2","NO2","NO3","Si")
# view(is.na(CCfull.all[,Nutcols]))
Drop = which(is.na(CCfull.all[,Nutcols]),arr.ind=T)[,1]
CCfull = CCfull.all[-Drop,]
dim(CCfull) # 6761  100
dim(CCfull)/dim(CCfull.all) #70%
...
unique(CCfull$simu) #already limited!
...
#turn off limit for nutrient exploration
...
# CCfull$inverseVol = 1/CCfull$bwVl_m3 #I don't know where this column came from (or where it went). 11/13
...
names(CCfull)
unique(CCfull$Sub_region)
unique(CCfull$Region)
CCfull$distIndex
CCfull$distBB.m
...
ggplot(data=plotCC, aes(x=Sub_region, y=Phphytn)) + geom_boxplot(aes(fill=Sub_region)) + facet_wrap(.~Zone) + theme_bw()
ggplot(data=plotCC, aes(x=Sub_region, y=NH4)) + geom_boxplot(aes(fill=Sub_region)) + facet_wrap(.~Zone) + theme_bw()
ggplot(data=plotCC, aes(x=Sub_region, y=PO4)) + geom_boxplot(aes(fill=Sub_region)) + facet_wrap(.~Zone) + theme_bw()
ggplot(data=plotCC, aes(x=Sub_region, y=NO3_NO2)) + geom_boxplot(aes(fill=Sub_region)) + facet_wrap(.~Zone) + theme_bw()
ggplot(data=plotCC, aes(x=Sub_region, y=NO2)) + geom_boxplot(aes(fill=Sub_region)) + facet_wrap(.~Zone) + theme_bw()
ggplot(data=plotCC, aes(x=Sub_region, y=NO3)) + geom_boxplot(aes(fill=Sub_region)) + scale_y_log10()  + facet_wrap(.~Zone) + theme_bw()
ggplot(data=plotCC, aes(x=Sub_region, y=Si)) + geom_boxplot(aes(fill=Sub_region)) + facet_wrap(.~Zone) + theme_bw()   
ggplot(data=plotCC, aes(x=Sub_region, y=SST)) + geom_boxplot(aes(fill=Sub_region)) + facet_wrap(.~Zone) + theme_bw()
ggplot(data=plotCC, aes(x=Sub_region, y=SSS)) + geom_boxplot(aes(fill=Sub_region)) + facet_wrap(.~Zone) + theme_bw()
...
ggplot(data=plotCC, aes(x=distIndex, y=Phphytn)) + geom_boxplot(aes(fill=distIndex)) + facet_wrap(.~Zone) + theme_bw()
ggplot(data=plotCC, aes(x=distIndex, y=NH4)) + geom_boxplot(aes(fill=distIndex)) + facet_wrap(.~Zone) + theme_bw()
ggplot(data=plotCC, aes(x=distIndex, y=PO4)) + geom_boxplot(aes(fill=distIndex)) + facet_wrap(.~Zone) + theme_bw()
ggplot(data=plotCC, aes(x=distIndex, y=NO3_NO2)) + geom_boxplot(aes(fill=distIndex)) + facet_wrap(.~Zone) + theme_bw()
ggplot(data=plotCC, aes(x=distIndex, y=NO2)) + geom_boxplot(aes(fill=distIndex)) + facet_wrap(.~Zone) + theme_bw()
ggplot(data=plotCC, aes(x=distIndex, y=NO3)) + geom_boxplot(aes(fill=distIndex))  + scale_y_log10() + facet_wrap(.~Zone) + theme_bw()
ggplot(data=plotCC, aes(x=distIndex, y=Si)) + geom_boxplot(aes(fill=distIndex)) + facet_wrap(.~Zone) + theme_bw()   #facet_wrap(.~Zone, ncol=1)
ggplot(data=plotCC, aes(x=distIndex, y=SST)) + geom_boxplot(aes(fill=distIndex)) + facet_wrap(.~Zone) + theme_bw()
ggplot(data=plotCC, aes(x=distIndex, y=SSS)) + geom_boxplot(aes(fill=distIndex)) + facet_wrap(.~Zone) + theme_bw()

CC_sf = st_as_sf(CCfull, coords = c("Longitude","Latitude"), crs = st_crs(4326))  
...
#       AIC(m_new,m_new1,sm_new)
      ...
      #REPLACE region interaction with nutrients
      m0.7=lm(RY~
        CALC_m2+
          ALGi_m2+
          SGi_m2+
          ...
          # SSS+
          # SST+
          #could also sub in SST and SSS (spatially averaged instead)
          Chla+NO3,
          # Chla+Phphytn+NH4+PO4+NO3_NO2+NO2+NO3+Si,
          # Chla+NO2+NO3+Si,
        data=CC.)
      sm0.7=summary(m0.7)
      sm0.7$adj.r.squared
...
model_SHAPE_scale  <- scale_shape_manual(name = "Model Parameters Used", values = model_shapes,
                                         ...
                                         labels = c("Benthic Only", 
                                                    ...
                                                    "BEN + HR + PAR + OCE + 1/VOL + DEP + MON + TEMP + SAL + NUTRIENTS"))
...
# ggsave(plot = ModPlot,filename = '/Users/heidi.k.hirsh/Desktop/Model+nutrients_Nov17.svg',units="in",width=12,height=9)
